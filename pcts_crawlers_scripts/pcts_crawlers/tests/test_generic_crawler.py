...
class GenericCrawlerTest(unittest.TestCase):
    ...
    def test_document_page_parse_from_incra(self):
        url = (
            "https://www.gov.br/incra/pt-br/assuntos/noticias/quilombolas"
            "-tratam-de-titulacao-e-acesso-a-creditos-com-gestores-do-incra-rs"
        )
        ...
        results = crawler.parse_page(
            fake_response_from_file('fixtures/incra/document_page.html', url),
            ("Quilombolas tratam de titulação e acesso a créditos com gestores "
             "do Incra/RS — Português (Brasil)"),
            True
        )

        responses_dir = os.path.dirname(os.path.realpath(__file__))
        file_path = os.path.join(
            responses_dir, 'responses/fixtures/incra/parsed_content.txt')
        file = open(file_path, 'r')

        expected_attributes = {
            "source": "incra",
            "url": url,
            "title": ("Quilombolas tratam de titulação e acesso a créditos com "
                      "gestores do Incra/RS — Português (Brasil)"),
            "content": file.read()
        }
        file.close()
    ...
    def test_document_page_parse_from_mpf(self):
        url = (
            "http://www.mpf.mp.br/atuacao-tematica/ccr6/grupos-de-trabalho-1/"
            "quilombos/legislacao/decretos/07_02_2007.pdf/view"
        )
        crawler = GenericCrawlerSpider(
            url_root=url,
            site_name="mpf",
        ...
        results = crawler.parse_page(
            fake_response_from_file('fixtures/mpf/document_page.html', url),
            ("Decreto de 07 de fevereiro de 2007 — 6ª Câmara - Populações "
        ...
        responses_dir = os.path.dirname(os.path.realpath(__file__))
        file_path = os.path.join(
            responses_dir, 'responses/fixtures/mpf/parsed_content.txt')
        file = open(file_path, 'r')

        expected_attributes = {
            "source": "mpf",
            "url": url,
            "title": ("Decreto de 07 de fevereiro de 2007 — 6ª Câmara - "
